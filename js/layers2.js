...
introBox: {
  title: "层级11--判定",
  body(){return "欢迎来到第11层，判定！你在本层级的目标是推进判定线和判定区间，提升谱面的难度和一些其他内容，同时，在本层会有很多对静态层级的增益。<br>解锁不了？把Rot升级树换成21-31-32-41-43-51-53-61-72-82-92-102-112-121-131-141-151-73-83-93-71-81再试试？(需要120Rot点数)"},
        },
judBox: {
  title: "判定区间",
...
        },
   ...
   if(challengeCompletions('ri',11)) exp=exp.mul(challengeEffect('ri',11))
    ...
    phi3() {
     let a=n(1)
     let b=player.ri.songs
     if(gba('ri',11).gte(4)) a=n(10).pow(b.mul(gba('ri',11).pow(2)).div(5))
     return a.max(1)
    },
    la1() {
     let a=n(1)
